...
{
	u8 block_bitmap_arr[1024] = {0};

	// Set the first 24 blocks as occupied
	block_bitmap_arr[0] = 0b11111111;
	block_bitmap_arr[1] = 0b11111111;
	block_bitmap_arr[2] = 0b01111111;
	block_bitmap_arr[127] = 0b10000000;
	// Set all bits after the first 1024 bits as occupied
	for (int i = 128; i < 1024; i++)
	{
		block_bitmap_arr[i] = 0b11111111;
	}
	...
	if (write(fd, &block_bitmap_arr, NUM_BLOCKS) != NUM_BLOCKS)
	{
...
{
	u8 inode_bitmap_arr[BLOCK_SIZE] = {0};

	inode_bitmap_arr[0] = 0b11111111;
	inode_bitmap_arr[1] = 0b00011111;
	for (int i = 16; i < 1024; i++)
	{
		inode_bitmap_arr[i] = 0b11111111;
	}

	off_t off = lseek(fd, BLOCK_OFFSET(INODE_BITMAP_BLOCKNO), SEEK_SET);
	if (off == -1)
	{
		errno_exit("lseek inode bitmap");
	}
	if (write(fd, &inode_bitmap_arr, NUM_BLOCKS) != NUM_BLOCKS)
	{
		errno_exit("write inode bitmap");
	}
}
...
void write_root_dir_block(int fd)
{
	...
	struct ext2_dir_entry child1_entry = {0};
	dir_entry_set(child1_entry, LOST_AND_FOUND_INO, "lost+found");
	dir_entry_write(child1_entry, fd);

	bytes_remaining -= child1_entry.rec_len;
